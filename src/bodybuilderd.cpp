...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/bodybuilder.conf are parsed in qt/bodybuilder.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  bodybuilderd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bodybuilderd -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for bodybuilderd but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect bodybuilderd signal handlers
    noui_connect();
