...
#include <qt/bodybuilderaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bodybuilder address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BodybuilderAddressEntryValidator(parent));
    widget->setCheckValidator(new BodybuilderAddressCheckValidator(parent));
}
...
bool parseBodybuilderURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bodybuilder: URI
    if(!uri.isValid() || uri.scheme() != QString("bodybuilder"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BodybuilderUnits::parse(BodybuilderUnit::BYLL, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBodybuilderURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBodybuilderURI(uriInstance, out);
}

QString formatBodybuilderURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bodybuilder:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BodybuilderUnits::format(BodybuilderUnit::BYLL, info.amount, false, BodybuilderUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBodybuilderConf()
{
    ...
    /* Open bodybuilder.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bodybuilder.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bodybuilder (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Bodybuilder (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Bodybuilder*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bodybuilder.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bodybuilder-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bodybuilder.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bodybuilder\n";
        else
            optionFile << strprintf("Name=Bodybuilder (%s)\n", chain);
