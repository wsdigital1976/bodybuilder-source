...
/** Call node.updateRwSetting() with Bodybuilder 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Bodybuilder 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/bodybuilder/bodybuilder/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bodybuilder 22.x will not try to read these.
...
//! Convert enabled/size values to bodybuilder -prune setting.
...
//! Get pruning enabled value to show in GUI from bodybuilder -prune setting.
...
//! Get pruning size value to show in GUI from bodybuilder -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBodybuilderUnit")) {
        settings.setValue("DisplayBodybuilderUnit", QVariant::fromValue(BodybuilderUnit::BYLL));
    }
    QVariant unit = settings.value("DisplayBodybuilderUnit");
    if (unit.canConvert<BodybuilderUnit>()) {
        m_display_bodybuilder_unit = unit.value<BodybuilderUnit>();
    } else {
        m_display_bodybuilder_unit = BodybuilderUnit::BYLL;
        settings.setValue("DisplayBodybuilderUnit", QVariant::fromValue(m_display_bodybuilder_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bodybuilder.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bodybuilder_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BodybuilderUnit>() == m_display_bodybuilder_unit) return;
    m_display_bodybuilder_unit = new_unit.value<BodybuilderUnit>();
    QSettings settings;
    settings.setValue("DisplayBodybuilderUnit", QVariant::fromValue(m_display_bodybuilder_unit));
    Q_EMIT displayUnitChanged(m_display_bodybuilder_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bodybuilder/bodybuilder/pull/8273
    ...
    // (https://github.com/bodybuilder-core/gui/issues/567).
