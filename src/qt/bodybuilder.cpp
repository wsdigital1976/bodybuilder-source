...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BodybuilderUnit>("BodybuilderUnit");
#else
    qRegisterMetaType<BodybuilderUnit>("BodybuilderUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bodybuilder_de.qm (shortcut "de" needs to be defined in bodybuilder.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bodybuilder_de_DE.qm (shortcut "de_DE" needs to be defined in bodybuilder.qrc)
...
static const char* qt_argv = "bodybuilder-qt";

BodybuilderApplication::BodybuilderApplication()
...
void BodybuilderApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BodybuilderApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BodybuilderGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BodybuilderApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BodybuilderApplication::createOptionsModel(bool resetSettings)
{
...
void BodybuilderApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BodybuilderGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BodybuilderGUI::quitRequested, this, &BodybuilderApplication::requestShutdown);
...
void BodybuilderApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BodybuilderApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BodybuilderApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BodybuilderApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BodybuilderApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BodybuilderApplication::handleRunawayException);
    connect(this, &BodybuilderApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BodybuilderApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BodybuilderApplication::parameterSetup()
{
...
void BodybuilderApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BodybuilderApplication::requestInitialize()
{
...
void BodybuilderApplication::requestShutdown()
{
...
void BodybuilderApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // bodybuilder: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BodybuilderGUI::handlePaymentRequest);
            connect(window, &BodybuilderGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BodybuilderApplication::handleRunawayException(const QString &message)
{
...
void BodybuilderApplication::handleNonFatalException(const QString& message)
{
...
WId BodybuilderApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BodybuilderApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BodybuilderGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bodybuilder);
    Q_INIT_RESOURCE(bodybuilder_locale);
    ...
    /// 6-7. Parse bodybuilder.conf, determine network, switch to network specific
    ...
    // bodybuilder: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
