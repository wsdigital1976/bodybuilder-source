This directory contains the source code for the Bodybuilder Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### bodybuildergui.(h/cpp)

- Represents the main window of the Bodybuilder UI.
...
- ClientModel is used by the main application `bodybuildergui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bodybuilder:... link from a browser).
...
* UI elements like BodybuilderAmountField, which inherit from QWidget.
* `bodybuilderstrings.cpp`: automatically generated
* `bodybuilderunits.(h/cpp)`: BYLL / mBYLL / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bodybuilder_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bodybuilder-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bodybuilder-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bodybuilder-qt`
9. You're all set! Start developing, building, and debugging the Bodybuilder Core GUI
