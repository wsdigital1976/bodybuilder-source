...
const char * const BODYBUILDER_CONF_FILENAME = "bodybuilder.conf";
const char * const BODYBUILDER_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bodybuilder-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BODYBUILDER_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Bodybuilder
    // macOS: ~/Library/Application Support/Bodybuilder
    // Unix-like: ~/.bodybuilder
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bodybuilder";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Bodybuilder";
#else
    // Unix-like
    return pathRet / ".bodybuilder";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BODYBUILDER_CONF_FILENAME));
}
