...
""" Example logging Bodybuilder Core mempool events using the mempool:added,
...
def main(bodybuilderd_path):
    bodybuilderd_with_usdts = USDT(path=str(bodybuilderd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bodybuilderd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bodybuilderd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bodybuilderd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bodybuilderd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bodybuilderd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/bodybuilderd")
        exit(1)
