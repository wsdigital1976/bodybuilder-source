# Bootstrappable Bodybuilder Core Builds

This directory contains the files necessary to perform bootstrappable Bodybuilder
Core builds.
...
To build Bodybuilder Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Bodybuilder Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bodybuilder-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bodybuilder-core/guix.sigs).
