...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bodybuilder-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bodybuilder-cli', help='bodybuilder-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Bodybuilder address to which the faucet should send')
...
parser.add_argument('bodybuilder_cli_args', nargs='*', help='Arguments to pass on to bodybuilder-cli (default: -signet)')

args = parser.parse_args()

if args.bodybuilder_cli_args == []:
    args.bodybuilder_cli_args = ['-signet']


def bodybuilder_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bodybuilder_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bodybuilder_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bodybuilder_cli(['getnewaddress', 'faucet', 'bech32'])
