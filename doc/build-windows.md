...
Below are some notes on how to build Bodybuilder Core for Windows.

The options known to work for building Bodybuilder Core on Windows are:
...
Note that for WSL the Bodybuilder Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bodybuilder, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bodybuilder`, for example:

    make install DESTDIR=/mnt/c/workspace/bodybuilder
