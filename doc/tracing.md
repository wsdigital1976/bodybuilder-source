# User-space, Statically Defined Tracing (USDT) for Bodybuilder Core

Bodybuilder Core includes statically defined tracepoints to allow for more
...
**Note**: Bodybuilder Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Bodybuilder Core
...
Multiple tools can list the available tracepoints in a `bodybuilderd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bodybuilder Core, use `info probes` in `gdb`:

```
$ gdb ./src/bodybuilderd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/bodybuilderd
stap net        outbound_message 0x0000000000107c05 /src/bodybuilderd
stap validation block_connected  0x00000000002fb10c /src/bodybuilderd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bodybuilder Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bodybuilderd | grep NT_STAPSDT -A 4 -B 2
