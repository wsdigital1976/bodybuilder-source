Bodybuilder Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/bodybuilder-core-0.18.0/>
...
`/Applications/Bodybuilder-Qt` (on Mac) or `bodybuilderd`/`bodybuilder-qt` (on
Linux).
...
recommended to use Bodybuilder Core on unsupported systems.

Bodybuilder Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Bodybuilder Core does not yet change appearance when
...
`contrib/init/bodybuilderd.service`, it has been changed to use
`/var/lib/bodybuilderd` as the data directory instead of
`~bodybuilder/.bodybuilder`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bodybuilderd` will
exist has enough space (check using `df -h /var/lib/bodybuilderd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bodybuilder/bodybuilder/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:13689:13689` (this is an extra `:13689` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Bodybuilder Core
...
The systemd init file (`contrib/init/bodybuilderd.service`) has been changed
to use `/var/lib/bodybuilderd` as the data directory instead of
`~bodybuilder/.bodybuilder`. This change makes Bodybuilder Core more consistent with
...
`datadir` option in `/etc/bodybuilder/bodybuilder.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bodybuilder/bodybuilder.conf`.


Documentation
-------------

- A new short [document](https://github.com/bodybuilder/bodybuilder/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bodybuilder/bodybuilder/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bodybuilder/bodybuilder/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bodybuilder/bodybuilder/blob/master/doc/bodybuilder-conf.md)
  about the `bodybuilder.conf` file describes how to use it to configure
  Bodybuilder Core.

- A new document introduces Bodybuilder Core's BIP174 [Partially-Signed
  Bodybuilder Transactions
  (PSBT)](https://github.com/bodybuilder/bodybuilder/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bodybuilder/bodybuilder/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bodybuilder-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bodybuilder Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bodybuilder/bodybuilder/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bodybuilder/bodybuilder/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bodybuilder Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bodybuilder Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bodybuilder/bodybuilder/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bodybuilder/bodybuilder/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bodybuilderd with the `-deprecatedrpc=generate`
...
- A new `bodybuilder-wallet` tool is now distributed alongside Bodybuilder
...
This section describes planned changes to Bodybuilder Core that may affect
other Bodybuilder software and services.

- Since version 0.16.0, Bodybuilder Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bodybuilder Core 0.20 (expected about
  a year after 0.18), Bodybuilder Core will default to native segwit
  ...
  to bech32 addresses, and if the Bodybuilder Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bodybuilder Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Bodybuilder Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bodybuilder Core
...
- A sub-project of Bodybuilder Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bodybuilder Core.  See their [project
  page](https://github.com/bodybuilder-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bodybuilder Core's own implementation, although entropy
  gathered by Bodybuilder Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bodybuilder Core a
...
- On macOS, Bodybuilder Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bodybuilder_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbodybuilder_server.a` (jnewbery)
...
- #13926 [Tools] bodybuilder-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bodybuilderd (MarcoFalke)
...
- #14521 qt, docs: Fix `bodybuilder-qt -version` output formatting (hebasto)
...
- #11625 Add BodybuilderApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbodybuilder_server` (laanwj)
...
- #15154 Configure: bodybuilder-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bodybuilder-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Bodybuilder Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbodybuilder_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bodybuilder-util-test.py (isghe)
...
- #12775 Integration of property based testing into Bodybuilder Core (Christewart)
...
- #12246 Bugfix: Only run bodybuilder-tx tests when bodybuilder-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bodybuilder` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bodybuilder` (MarcoFalke)
...
- #14186 bodybuilder-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bodybuilder.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bodybuilder.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bodybuilder-conf.md (hebasto)
...
- #15354 Add missing `bodybuilder-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bodybuilderconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bodybuilder folder on OSX (JimmyMow)
