# PSBT Howto for Bodybuilder Core

Since Bodybuilder Core 0.17, an RPC interface exists for Partially Signed Bodybuilder
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bodybuilder/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Bodybuilder transactions that are not fully signed
...
#### Multisig with multiple Bodybuilder Core instances
...
Bodybuilder Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bodybuilder-cli` is used.
...
Later, when *V* BYLL has been received on *Amulti*, and Bob and Carol want to
