# CJDNS support in Bodybuilder Core

It is possible to run Bodybuilder Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bodybuilder Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bodybuilder network and individual nodes.
...
before it will work with your Bodybuilder Core node. This is described in steps
...
## Run Bodybuilder Core with CJDNS

Once you are connected to the CJDNS network, the following Bodybuilder Core
...
When enabled, this option tells Bodybuilder Core that it is running in an
...
IPv6 local network. This helps Bodybuilder Core perform better address management:
...
CJDNS support was added to Bodybuilder Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bodybuilder-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/bodybuilder/bodybuilder/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Bodybuilder Core

There are several ways to see your CJDNS address in Bodybuilder Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bodybuilder-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bodybuilder-cli getpeerinfo`).
