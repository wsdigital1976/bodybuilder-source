This directory contains integration tests that test bodybuilderd and its
...
bodybuilderd and bodybuilder-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bodybuilder-util, bodybuilder-tx, ...).
...
Before tests can be run locally, Bodybuilder Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the bodybuilderd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bodybuilderd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bodybuilderd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bodybuilderd processes are running.

On linux, the test framework will warn if there is another
bodybuilderd process running when the tests are started.

If there are zombie bodybuilderd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bodybuilderd processes running on the system, so should not be used if any non-test
bodybuilderd processes are being run.**

```bash
killall bodybuilderd
```

or

```bash
pkill -9 bodybuilderd
```
...
sure bodybuilderd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bodybuilderd `debug.log`s will all be dumped
...
`test_framework.log` and bodybuilderd `debug.log`s can be combined into a single
...
call methods that interact with the bodybuilderd nodes-under-test.

If further introspection of the bodybuilderd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bodybuilderd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bodybuilderd <pid>
```
