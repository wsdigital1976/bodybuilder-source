...
TMPDIR_PREFIX = "bodybuilder_func_test_"
...
class BodybuilderTestMetaClass(type):
    """Metaclass for BodybuilderTestFramework.

    Ensures that any attempt to register a subclass of `BodybuilderTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BodybuilderTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BodybuilderTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BodybuilderTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BodybuilderTestFramework(metaclass=BodybuilderTestMetaClass):
    """Base class for a bodybuilder test script.

    Individual bodybuilder test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bodybuilderds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bodybuilderds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bodybuilder-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bodybuilderd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bodybuilderd" + config["environment"]["EXEEXT"],
        )
        fname_bodybuildercli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bodybuilder-cli" + config["environment"]["EXEEXT"],
        )
        fname_bodybuilderutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bodybuilder-util" + config["environment"]["EXEEXT"],
        )
        self.options.bodybuilderd = os.getenv("BODYBUILDERD", default=fname_bodybuilderd)
        self.options.bodybuildercli = os.getenv("BODYBUILDERCLI", default=fname_bodybuildercli)
        self.options.bodybuilderutil = os.getenv("BODYBUILDERUTIL", default=fname_bodybuilderutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bodybuilderds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BodybuilderRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bodybuilderd', self.options.bodybuilderd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bodybuilder-cli', self.options.bodybuildercli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                bodybuilderd=binary[i],
                bodybuilder_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bodybuilderd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bodybuilderd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bodybuilderd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bodybuilderd=self.options.bodybuilderd,
                    bodybuilder_cli=self.options.bodybuildercli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bodybuilder.conf
    ...
    def skip_if_no_bodybuilderd_tracepoints(self):
        """Skip the running test if bodybuilderd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bodybuilderd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bodybuilderd_zmq(self):
        """Skip the running test if bodybuilderd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bodybuilderd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bodybuilder-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bodybuilder-wallet has not been compiled")

    def skip_if_no_bodybuilder_util(self):
        """Skip the running test if bodybuilder-util has not been compiled."""
        if not self.is_bodybuilder_util_compiled():
            raise SkipTest("bodybuilder-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bodybuilder-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bodybuilder-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bodybuilder-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bodybuilder-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bodybuilder_util_compiled(self):
        """Checks whether bodybuilder-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BODYBUILDER_UTIL")
