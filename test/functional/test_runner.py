...
`test/functional/test_framework/test_framework.py:BodybuilderTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_bodybuilder_cli.py --legacy-wallet',
    'interface_bodybuilder_cli.py --descriptors',
...
def main():
    ...
    enable_bodybuilderd = config["components"].getboolean("ENABLE_BODYBUILDERD")

    if not enable_bodybuilderd:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if bodybuilderd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "bodybuilderd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a bodybuilderd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `bodybuilder-cli help` (`rpc_interface.txt`).
